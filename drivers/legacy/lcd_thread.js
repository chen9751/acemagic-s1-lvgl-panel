...
function main(state) {
    ...
    node_hid.devices().find(each => {
        ...
        let hid_device = state.device;

    let devices = node_hid.devices();

    let s1 = devices.find(each => {
        return (
            each.vendorId === 1241 &&
            each.productId === 64769 &&
            each.interface === 1
        );
    });
    ...
    if (s1) {
        logger.info(
            "AUTO FOUND S1: " + JSON.stringify(s1)
        );
        hid_device = s1.path;
    }
    usb_hid.open(hid_device).then(handle => {

        logger.info("LCD OPEN SUCCESS: " + state.device);
